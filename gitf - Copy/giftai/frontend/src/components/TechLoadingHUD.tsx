...
const GlitchText = ({ text, className }: { text: string; className?: string }) => {
  return (
    <div className={`relative ${className}`}>
      ...
      <span className="absolute top-0 left-[1px] -z-10 opacity-50 animate-pulse text-red-500">{text}</span>
      <span className="absolute top-0 -left-[1px] -z-10 opacity-50 animate-pulse text-blue-500 delay-75">{text}</span>
    </div>
  );
};
...
const HexData = ({ position }: { position: string }) => {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setData(`0x${Math.floor(Math.random() * 255).toString(16).toUpperCase().padStart(2, '0')}`);
    }, 100 + Math.random() * 200);
    return () => clearInterval(interval);
  }, []);

  return (
    <div className={`absolute ${position} font-mono text-xs text-cyan-500/70 tracking-widest`}>
      DATA_STREAM: {data}
    </div>
  );
};
  ...
  return (
    <group>
      ...
      <mesh scale={0.8}>
         ...
         <meshBasicMaterial color="#00ffff" transparent opacity={0.2} wireframe={false} blending={THREE.AdditiveBlending} />
      </mesh>
    </group>
  );
};
...
const AIProcessTerminal = ({ progress }: { progress: number }) => {
  ...
  useEffect(() => {
    if (progress > 5 && !logs.includes("> CONNECTING_TO_SATELLITE...")) setLogs(p => [...p, "> CONNECTING_TO_SATELLITE..."]);
    if (progress > 20 && !logs.includes("> HANDSHAKE_ESTABLISHED")) setLogs(p => [...p, "> HANDSHAKE_ESTABLISHED"]);
    if (progress > 40 && !logs.includes("> DOWNLOADING_NEURAL_MAP...")) setLogs(p => [...p, "> DOWNLOADING_NEURAL_MAP..."]);
    if (progress > 60 && !logs.includes("> DECRYPTING_GIFT_MATRIX...")) setLogs(p => [...p, "> DECRYPTING_GIFT_MATRIX..."]);
    if (progress > 80 && !logs.includes("> OPTIMIZING_HOLOGRAM_CORE...")) setLogs(p => [...p, "> OPTIMIZING_HOLOGRAM_CORE..."]);
    if (progress > 95 && !logs.includes("> FINALIZING_RENDER_SEQUENCE")) setLogs(p => [...p, "> FINALIZING_RENDER_SEQUENCE"]);
  }, [progress, logs]);

  return (
    <div className="absolute top-20 left-10 w-64 font-mono text-xs text-cyan-400/80 space-y-1">
      <div className="border-b border-cyan-500/30 pb-1 mb-2 tracking-widest text-cyan-200">SYSTEM_LOGS</div>
      <AnimatePresence>
...
const SmartCountdownRing = () => {
    ...
    useEffect(() => {
        const interval = setInterval(() => setMs(prev => (prev <= 10 ? 999 : prev - 13)), 50);
        return () => clearInterval(interval);
    }, []);
    ...
    return (
        <div className="absolute bottom-20 right-10 flex items-center justify-center">
            {/* Rotating Outer Ring */}
            <div className={`w-24 h-24 rounded-full border border-cyan-500/30 border-t-cyan-400 animate-[spin_3s_linear_infinite] absolute`} />
            {/* Pulse Inner Ring */}
            <div className={`w-20 h-20 rounded-full border border-cyan-500/10 animate-ping absolute`} />
            ...
            <div className="flex flex-col items-center z-10 font-mono text-cyan-300">
                ...
                <span className="text-xs text-cyan-500">{ms.toString().padStart(3, '0')}ms</span>
            </div>
            
            <div className="absolute top-[-10px] bg-[#050510] px-1 text-[0.6rem] text-cyan-500">TIMER</div>
        </div>
    );
};
...
const FlashOverlay = ({ progress }: { progress: number }) => {
    ...
    useEffect(() => {
        ...
        if (Math.abs(progress - 25) < 1 || Math.abs(progress - 50) < 1 || Math.abs(progress - 75) < 1) {
             setFlash(true);
